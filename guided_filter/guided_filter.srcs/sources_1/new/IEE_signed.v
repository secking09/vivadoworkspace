...
module IEE_signed#( parameter IMAGE_WIDTH = 8,
                      parameter RADIUS = 15,
  ...
  (
    ...
    input reset,
    // Pixel Input
    input signed  [IMAGE_WIDTH-1:0] Iq,  // Leftover pixel
    input signed  [IMAGE_WIDTH-1:0] Is,  // Current pixel
    input valid_in,              // Valid_in
    // Window Sum Result
    output reg signed [SUM_BITS-1:0] SigIi, // Box Filter Sum
    ...
    input signed [INTEGRAL_WIDTH-1:0] rdata1,
    input signed [INTEGRAL_WIDTH-1:0] rdata2
  ...
  reg signed [INTEGRAL_WIDTH-1:0] II_upper_row, II_upper_row_next;
  reg signed [INTEGRAL_WIDTH-1:0] IIs;
  reg signed [INTEGRAL_WIDTH-1:0] IId; //IId <= IIs
  reg signed [INTEGRAL_WIDTH-1:0] prev_window, prev_window_nxt;
