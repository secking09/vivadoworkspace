...
module coeff_top_tb;
  ...
  // Output register
  wire signed [4+FRACBITS-DIVIDER_FRAC-1:0] ak;
  wire signed [SIGI_BITS+FRACBITS+SIGI_BITS+2:0] bk;
  wire valid_out; 
  ...
  coeff_top #(RADIUS, FRACBITS,DIVIDER_FRAC ) uut (
    .clk(clk),
    .reset_n(reset_n),
    .valid_in(valid_in),
    .valid_out(valid_out),
  ...
  initial begin
    ...
    for (i = 0; i < 502; i = i + 1) begin // Assuming 492 lines
      valid_in = 1; // Set valid_in high to start processing
      for (j = 0; j < 502; j = j + 1) begin // Assuming image width is 492
